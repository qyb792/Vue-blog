var github_canlendar = (git_user, git_color) => {
  var git_githubapiurl = 'https://python-github-calendar-api-jade.vercel.app/api?' + git_user
  ...
  function responsiveChart() {
    ...
    function getMousePos(canvas, event) {
      ...
      for (var item of git_positionplusdata) {
        var lenthx = x - item.x
        var lenthy = y - item.y
        if (lenthx > 0 && lenthx < lineminwitdh) {
          if (lenthy > 0 && lenthy < lineminwitdh) {
            git_span1 = item.date
